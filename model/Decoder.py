...
class TransformerDecoderBlock(nn.Module):
    def __init__(self, embedding_dim: int = 512, num_heads: int =12, forward_dim: int =1024, dropout: float = 0.1, max_seq_len: int = 100) -> None:
        super().__init__()
        ...
        self.register_buffer('mask_attn', torch.triu(torch.ones(max_seq_len, max_seq_len), diagonal=1).bool())
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        mask_attn_output, _ = self.multihead_attention1(x, x, x, attn_mask=self.mask_attn)
...
class TransformerDecoder(nn.Module):
    ...
    """
    def __init__(self, embedding_dim: int = 512, num_heads: int =12, forward_dim: int =1024, dropout: float = 0.1, max_seq_len: int = 100, num_layers: int =12, vocab_size: int = 1000) -> None:
        super().__init__()
        self.embedding_dim = embedding_dim
        self.num_heads = num_heads
        self.forward_dim = forward_dim
